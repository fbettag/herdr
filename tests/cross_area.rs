...
fn cross_area_detach_and_reattach_preserves_state() {
    ...
    client_handshake(&mut client_a, EXPECTED_PROTOCOL_VERSION, 100, 30);
    ...
    client_handshake(&mut client_b, EXPECTED_PROTOCOL_VERSION, 80, 24);
    assert!(
...
fn cross_area_client_and_api_workspace_views_are_consistent() {
    ...
    client_handshake(&mut client, EXPECTED_PROTOCOL_VERSION, 100, 30);
...
fn cross_area_two_clients_shared_view_and_single_detach_stability() {
    ...
    let mut client_a = UnixStream::connect(&client_socket).expect("client A should connect");
    client_handshake(&mut client_a, EXPECTED_PROTOCOL_VERSION, 110, 30);
    let mut client_b = UnixStream::connect(&client_socket).expect("client B should connect");
    client_handshake(&mut client_b, EXPECTED_PROTOCOL_VERSION, 100, 30);
...
fn cross_area_server_kill_then_restart_and_reconnect() {
    ...
    client_handshake(&mut reconnect_client, EXPECTED_PROTOCOL_VERSION, 80, 24);
    assert!(
